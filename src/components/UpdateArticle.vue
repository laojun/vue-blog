...
<script>
    export default {
      ...
      computed:{
        ...
        // title(){
        //   return this.initalData.title ? this.initalData.title : ""
        // },
        // content(){
        //   return this.initalData.content ? this.initalData.content : "";
        // },
        // chooseTag(){
        //   return this.initalData.chooseTag ? this.initalData.chooseTag : [];
        // }
      },
      methods:{
        add(){
          var params = {
            artId:this.initalData._id,//查询条件
            title:this.title,
            date:new Date(),
            tags:this.chooseTag,
          ...
          this.$http.post('/api/article/update',params)
              .then(
                (resolve) => {
                  console.log("update article suc");
                  this.tip = "修改成功";
                  this.addEnd = true;
                },
                (reject) => {
                  console.log("update article failed : "+reject);
                  this.tip = "修改失败";
                  this.addEnd = true;
                }
              ).catch((reject) => {
                console.log("update article failed : reject="+reject);
                this.tip = "修改失败";
                this.addEnd = true;
              });
        },
        ...
        getOne(){
          var params = {id:this.queryId};
          this.$http.get('/api/article/getOne',{params})
              .then(
                (res) => {
                  console.log("getOne article suc");
                  //this.tip = "添加成功";
                  //this.addEnd = true;
                  this.initalData = res.body;
                  this.title = this.initalData.title;
                  this.content = this.initalData.content;
                  this.chooseTag = this.initalData.tags;
                  
                  var tagsStr = "&"+this.tags.join("&")+"&";
                  for(var i=0;i<this.chooseTag.length;i++){
                      for(var j=0;j<this.tags.length;j++){
                        if(this.chooseTag[i]===this.tags[j].name){
                          this.$set(this.chsTag,'chs'+j , true);
                        }
                      }
                  }
                ...
                (res) => {
                  console.log("getOne article failed : "+reject);
                }
              ).catch((reject) => {
                console.log("getOne article failed : reject="+reject);
              });
        },
        addTag(){
